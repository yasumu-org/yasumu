...
export interface EventsCommon {
  ...
  listen<T extends unknown[]>(event: string, callback: EventCallback<T>): Promise<Callback>;
  once<T extends unknown[]>(event: string, callback: EventCallback<T>): Promise<Callback>;
}
